  ...
  return (
    <nav style={rail}>
      {workspaces.map((w) => (
        <div
          key={w.id}
          style={iconStyle(w.id === activeWorkspace?.id)}
          onClick={() => select(w)}
          title={w.name}
        >
          {w.iconUrl
            ? <img src={w.iconUrl} alt={w.name} style={{ width: 48, height: 48, borderRadius: "inherit", objectFit: "cover" }} />
            : w.name[0].toUpperCase()}
        </div>
      ))}
      <div style={divider} />
      <div
        style={{ ...iconStyle(false), color: "var(--online)", fontSize: 24 }}
        onClick={createWs}
        title="Add workspace"
      >
        +
