...
export function ServerList() {
  ...
  return (
    <TooltipProvider delay={300}>
      <nav className="flex flex-col items-center bg-zinc-900 w-[72px] h-full py-3 gap-2 shrink-0">
        <ScrollArea className="flex flex-col items-center gap-2 w-full">
          {workspaces.map((w) => {
            const active = w.id === activeWorkspace?.id;
            return (
              <Tooltip key={w.id}>
                <TooltipTrigger>
                  <div className="relative flex justify-center w-full mb-2 group cursor-pointer" onClick={() => select(w)}>
          ...
          <Tooltip>
            <TooltipTrigger>
              <div
                className="w-12 h-12 rounded-full bg-zinc-700 hover:bg-green-600 hover:rounded-2xl flex items-center justify-center cursor-pointer transition-all duration-150 text-green-400 hover:text-white"
                onClick={createWs}
              >
